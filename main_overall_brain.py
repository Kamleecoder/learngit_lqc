...
import os
os.environ["CUDA_DEVICE_ORDER" ] ="PCI_BUS_ID"   # see issue #152123
...
def topo_att_all_attention(h_1, h_2):
    ...
    T = h_1.get_shape()[1].value
    M = h_1.get_shape()[2].value

    h1_tmp, h2_tmp = tf.reshape(h_1, [-1, T*M]), tf.reshape(h_2, [-1, T*M]) # 6400*TM
    h_comb = tf.concat([tf.expand_dims(h1_tmp, 1), tf.expand_dims(h2_tmp, 1)], 1) # 6400*2*TM
    temp = tf.tanh(tf.einsum('ij,jkl->ikl', V_tp, tf.transpose(h_comb, perm=[2,1,0]))) # L_tp*2*6400
    A = tf.nn.softmax(tf.einsum('ij,jkl->ikl', w_tp, temp), dim=1) #1*2*6400

    return tf.transpose(A, perm=[2,1,0]) #6400*2*1
...
def mi_gru(Fea_batch, Topo_batch, idx_batch, samples_idx_batch, support_sizes_batch, n_hidden_units, num_stacked_layers, weights, biases, t_dim):
    ...
    h_1_tmp = tf.multiply(tf.transpose(tf.expand_dims(Gamma[:,0,:], 1), perm=[0,2,1]), h_list_tensor) # 6400*1*1 * 6400*T*M = 6400*T*M
    h_2_tmp = tf.multiply(tf.transpose(tf.expand_dims(Gamma[:,1,:], 1), perm=[0,2,1]), h_list_tensor_topo) # 6400*1*1 * 6400*T*M = 6400*T*M
...
if __name__ == '__main__':
    filedpath = '/15989546438/lqc/zq/Epinions/data'
    filename  = '/Epinion/epinion.npz'
    ...
    weights_att = {'W': tf.get_variable('Weights_W', shape=[r,L], dtype=tf.float64, initializer=tf.truncated_normal_initializer(mean=-0.1, stddev=0.1)), \
                    'V': tf.get_variable('Weights_V', shape=[L,2*M], dtype=tf.float64, initializer=tf.truncated_normal_initializer(mean=-0.1, stddev=0.1)), \
                        ...
                        'V1_h0': tf.get_variable('Weights_V1_h0', shape=[L10,L11], dtype=tf.float64, initializer=tf.truncated_normal_initializer(mean=-0.1, stddev=0.1)), \
                            ...
                            'V_tp': tf.get_variable('Weights_V_tp', shape=[L_tp,n_steps*M], dtype=tf.float64, initializer=tf.truncated_normal_initializer(mean=-0.1, stddev=0.1))}
